...
async def probe_messages():
    """Respond to Claude compatibility probes for the messages endpoint. (Auth removed for probes)"""
...
async def probe_count_tokens():
    """Respond to Claude compatibility probes for the token count endpoint. (Auth removed for probes)"""
    return _probe_response("POST, HEAD, OPTIONS")


@router.get("/")
async def root(settings: Settings = Depends(get_settings)):
    """Root endpoint. (Auth removed to allow connectivity checks)"""
    return {
        "status": "ok",
...
async def probe_root():
    """Respond to compatibility probes for the root endpoint. (Auth removed for probes)"""
